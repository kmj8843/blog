...
이 글이 필요한 이유는, `win 502` 같은 숫자를 봐도 그게 **속도 제한 신호인지**, **재전송 이야기인지**, **상대 버퍼 여유 이야기인지** 자꾸 섞이기 쉽기 때문이에요.

- ACK는 잘 가는데 왜 `Window` 값이 작아지는지
- 왜 네트워크가 멀쩡해도 상대가 덜 받으면 **보내는 속도가 줄어드는지**
- 왜 16비트 Window만 보면 안 되고 **Window Scale** 까지 같이 봐야 하는지
...
## 그래서 Window는 한마디로 뭐예요?

TCP의 Window는 **받는 쪽이 지금 더 받아둘 수 있는 자리**를 계속 알려주는 값이에요.

중요한 건, 이 값이 **길이 막혔다는 뜻**이 아니라는 점이에요. 이건 어디까지나 **수신자 버퍼 여유** 이야기예요.
