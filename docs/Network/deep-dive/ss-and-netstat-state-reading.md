...
바로 그 질문에 답하는 글이에요. 오늘은 `ss` 와 `netstat` 화면에서 **TCP 상태를 장면처럼 읽는 감각**을 잡아볼게요. 여기서는 모든 옵션을 백과사전처럼 다 모으기보다, **운영 화면에서 상태 줄이 보였을 때 무엇부터 의심하고 무엇은 성급히 단정하면 안 되는지** 쪽에 집중할 거예요.
...
이번에는 건물 관리실의 **출입 현황판**을 떠올려볼까요?

- `LISTEN` 은 **문을 열어두고 손님을 기다리는 상태**예요.
- `ESTABLISHED` 는 **손님이 들어와서 실제로 대화가 진행 중인 상태**예요.
- `TIME-WAIT` 는 **손님은 나갔지만, 문이 진짜 잘 닫혔는지 잠깐 더 확인하는 상태**예요.
- `CLOSE-WAIT` 는 **상대는 먼저 끝냈는데, 우리 쪽 정리가 아직 안 끝난 상태**예요.

즉 `ss` 나 `netstat` 는 패킷을 한 줄씩 보여주는 CCTV라기보다, **각 문이 지금 어떤 상황에 놓여 있는지 적어놓은 현황판**에 더 가까워요.
...
그다음엔 자연스럽게 이런 궁금증이 생길 거예요.

> *"좋아요, 상태 이름까지는 읽히기 시작했어요. 근데 그 연결 안에서 TLS는 정확히 어떤 순서로 손을 맞잡고 있을까요?"*

다음 글에서는 **TLS 핸드셰이크 장면**으로 넘어가서, 평문 TCP 연결 위에 보호 통로가 어떻게 덧씌워지는지 같이 열어볼게요.
