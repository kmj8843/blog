...
근데 여기서 한 걸음 더 가면 또 이런 생각이 들어요.

> *"좋아요, 이름과 역할은 알겠어요. 근데 실제 장면처럼 따라가면 어느 단계에서 무슨 일이 벌어지는 거죠?"*

바로 그 질문에 답하는 글이에요. 오늘은 **가장 흔한 TLS 1.3 인증서 기반 핸드셰이크 한 장면**을 기준으로, TCP가 열린 뒤부터 첫 HTTP 요청이 올라가기 전까지를 **어떤 순서로 보고, 어디서부터 읽어야 하는지** 따라가볼게요. 설명의 큰 뼈대는 [RFC 8446 2장과 4장](https://www.rfc-editor.org/rfc/rfc8446.html#section-2), 실제 흐름 감각은 [RFC 8448의 simple 1-RTT example](https://www.rfc-editor.org/rfc/rfc8448.html#section-3) 쪽을 바탕으로 잡아볼게요.

!!! note "이 글의 범위"
    여기서는 **현대 웹에서 가장 흔한 TLS 1.3의 기본 핸드셰이크 장면을 읽는 감각**에 집중할게요. 각 메시지 필드와 변형 흐름 자체를 해부하는 일은 [TLS 1.3 핸드셰이크는 실제로 어떤 순서일까요?](./tls13-handshake-anatomy.md#message-summary){ data-preview } 쪽이 맡고, 여기서는 **그 구조가 실제 줄 위에서 어떤 순서와 신호로 보이는지**를 붙잡을 거예요.

---

## 일단 비유로 시작해볼게요

이번에는 **예약된 상담실 앞에 남은 출입 기록**을 떠올려볼까요?

- 출입 기록에는 **누가 먼저 말을 꺼냈는지** 남고,
- **상대가 그 제안을 받아들였는지** 남고,
- 그 뒤엔 **문 안쪽 단계로 넘어갔는지** 흔적이 남고,
- 마지막으로 **확인 절차가 닫혔는지** 가 순서대로 남아요.

기본편에서는 이 과정을 **보호된 통로를 준비하는 큰 그림**으로 봤다면, 구조 글에서는 **각 메시지가 무슨 역할인지**를 봤죠. 오늘은 그걸 **복도 기록지처럼 한 줄씩 읽는 감각**으로 다시 볼 거예요.

| 기본편에서 잡은 감각 | 비유에서는 | 실제로는 |
|---|---|---|
| 보호된 통로 준비 | 상담실 출입 기록 | TLS 핸드셰이크 흔적 |
| 누가 먼저 시작했는지 | 방문객이 먼저 요청 남김 | `ClientHello` |
| 경계가 바뀌는 지점 | 상담실 쪽이 방식 확정 | `ServerHello` |
| 안쪽 단계 진입 | 문 안쪽 기록으로 넘어감 | `EncryptedExtensions` 이후 |
| 마지막 확인 | 확인 절차 닫힘 | `Finished` |
