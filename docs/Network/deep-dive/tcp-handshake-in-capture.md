...
- 지금 이 세 줄이 **정상 연결 시작**인지
- 아니면 `SYN` 만 반복되는 **재전송 장면**인지
- 아니면 `SYN-ACK` 까지는 오는데 마지막 ACK가 안 보여서 **어디선가 끊기는지**

그 판단이 안 서면 캡처를 들여다봐도 여전히 답답하거든요. 오늘은 `tcpdump` 화면에서 **SYN, SYN-ACK, ACK가 실제로 어떻게 찍히는지**, 그리고 **어디서 끊기면 무엇을 의심해야 하는지**까지 같이 볼게요.
...
이 글에서 보는 건 새로운 프로토콜이 아니에요. **이미 알고 있는 TCP handshake를 캡처 로그 위에서 다시 읽는 장면**이에요.
