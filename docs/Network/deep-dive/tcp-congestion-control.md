...
바로 그 장면을 여는 글이에요. 오늘은 **수신자 버퍼 여유를 보는 흐름 제어**와, **네트워크 길 자체가 붐비는지 보며 송신 속도를 조절하는 혼잡 제어**를 나눠서 보고, `cwnd`, slow start, congestion avoidance, duplicate ACK 같은 말을 초심자 눈높이에서 같이 정리해볼게요.
...
이번에는 차를 보내는 **물류센터 출구 도로**를 떠올려볼까요?

- 창고 안에 실을 상자는 아주 많아요.
- 받는 쪽 창고도 아직 받아둘 여유가 있을 수 있어요.
- 근데 출구 도로가 이미 꽉 막혀 있으면, 차를 계속 밀어 넣는다고 더 빨리 도착하진 않겠죠.
- 오히려 진입로까지 엉키고, 중간에 사고도 더 잘 날 거예요.
