...
이번에는 국제 택배 송장을 떠올려볼까요?

- IPv4 송장은 예전부터 쓰던 양식이라, 칸이 많고 경우에 따라 **추가 메모**도 안쪽에 같이 적어요.
- IPv6 송장은 그걸 보고 한 번 정리한 새 양식이에요.
- 그래서 **맨 앞 송장 자체는 무조건 같은 길이**로 맞추고,
- 길게 써야 하는 특별 지시는 **뒤에 덧붙이는 별도 메모지**로 빼버렸어요.
