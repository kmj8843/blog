...
!!! note "이 글의 범위"
    여기서는 **IPv4 기본 헤더(옵션 없는 20바이트짜리)** 만 다뤄요. IPv6 헤더와 IPv4 옵션 영역은 별도 글에서 따로 열어요. 그리고 이 글은 비트 단위로 들어가는 첫 글이라, 이후 deep-dive 글들이 "이 비트가 어디 있더라" 할 때 자주 돌아오게 될 거예요.
...
- 같은 자리에 다른 모양으로 다시 그려진 헤더가 보고 싶다면 — IPv6 헤더 해부 (예정)
- 이 IP 헤더 바로 위에 얹히는 TCP 헤더가 어떻게 생겼는지 보고 싶다면 — TCP 헤더 해부 (예정)
- 격자가 아니라 실제 캡처 한 줄 위에서 IP 헤더 값을 읽고 싶다면 — `tcpdump` 첫인상 (예정)

> 다음 deep-dive 글에서는 같은 격자 자리에 IPv4 와 IPv6 가 얼마나 다르게 들어앉는지를 같이 펼쳐볼게요.
