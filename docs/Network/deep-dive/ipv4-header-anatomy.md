...
[IP 주소와 라우팅](../basic/02-ip-and-routing.md){ data-preview }에서는 IP 헤더를 **카드 4장**(출발지 IP / 도착지 IP / TTL / 프로토콜) 으로만 보여줬어요. 흐름을 잡기에는 그걸로 충분했지만, *"그래서 그 카드 안은 진짜로 어떻게 생겼는데?"* 가 비어 있었죠.

오늘 글은 그 카드 안을 열어볼 거예요. 카드 4장이 사실은 **32비트 줄 다섯 개에 13개 필드가 빽빽이 들어찬 격자** 였다는 사실을 직접 보고, 한 줄씩 같이 읽어 내려갈 거예요.

!!! note "이 글의 범위"
    여기서는 **IPv4 기본 헤더(옵션 없는 20바이트짜리)** 만 다뤄요. IPv6 헤더와 IPv4 옵션 영역은 별도 글에서 따로 열어요. 그리고 이 글은 비트 단위로 들어가는 첫 글이라, 이후 deep-dive 글들이 "이 비트가 어디 있더라" 할 때 자주 돌아오게 될 거예요. RFC 기준으로는 [RFC 791](https://www.rfc-editor.org/rfc/rfc791) 의 IPv4 헤더 형식을 바탕으로 읽어갈 거예요.

---

## 일단 비유로 시작해볼게요

[IP 주소와 라우팅 — 그럼 진짜 IP 헤더는 어떻게 생겼을까요?](../basic/02-ip-and-routing.md#ip-header-card){ data-preview } 에서 봤던 그 카드, 다시 떠올려볼게요.

기본편에서는 **출발지 IP / 도착지 IP / TTL / 프로토콜** 카드 네 장만 손에 들고 있었죠. 오늘은 그 카드를 **실제 헤더 격자** 위에 다시 올려놓는 편이라고 생각하면 돼요.
...
그러니까 이 글은 **새 개념을 더 만든다기보다**, 기본편에서 이미 본 카드 네 장이 **헤더 안의 몇 번째 바이트, 몇 비트 칸에 들어 있는지** 다시 연결해주는 글이에요.
...
이 헤더 격자는 다른 deep-dive 글에서 자주 다시 등장해요.

- 같은 자리에 다른 모양으로 다시 그려진 헤더가 보고 싶다면 — [IPv6 헤더는 왜 딱 40바이트일까요?](./ipv6-header-anatomy.md){ data-preview }
- 이 IP 헤더 바로 위에 얹히는 TCP 헤더가 어떻게 생겼는지 보고 싶다면 — TCP 헤더 해부 (예정)
