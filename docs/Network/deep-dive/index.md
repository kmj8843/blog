...
먼저 [HTTP/1.1 메시지의 시작 줄, 헤더, 빈 줄, 본문 구조](./http1-message-grammar.md){ data-preview }를 보면, 브라우저와 서버가 실제로 어떤 모양의 약속문을 주고받는지부터 잡을 수 있어요. 그다음에야 waterfall, 프록시, 캐시, 오리진 지연 같은 운영 장면을 더 정확히 읽기 쉬워져요.
