...
!!! note "이 글의 범위"
    여기서는 **tcpdump 한 줄을 읽는 첫 감각**과, 그 감각을 바로 연습할 수 있는 **자주 쓰는 옵션 / 필터 조합**까지 다뤄요. 다만 BPF 문법 전체를 레퍼런스처럼 다 모으거나, `.pcap` 파일을 Wireshark에서 흐름 단위로 다시 해부하는 단계까지는 아직 안 들어갈 거예요. 기본편에서 봤던 패킷 캡처의 큰 그림을, 이제 **줄 단위 관찰 로그** 위에 다시 올려보는 첫 심화편이라고 생각하면 돼요.
...
아니에요. 사람이 읽기 좋게 **요약해서 보여주는 표현**이에요. 더 자세한 바이트나 헤더 칸은 옵션을 더 주거나, Wireshark 같은 도구로 다시 열어봐야 할 수 있어요.
...
여기까지 오면 보통 이런 궁금증이 자연스럽게 이어져요.

> *"좋아요, tcpdump 한 줄은 읽겠어요. 그럼 저장한 `.pcap` 파일을 Wireshark에서 열면, 이 한 줄 뒤의 전체 대화를 어떻게 따라가죠?"*

그 관심사는 자연스럽게 **패킷 흐름을 화면으로 따라가는 장면** 쪽으로 이어져요. tcpdump가 줄 단위 자막이라면, 그다음은 **흐름 전체를 따라붙는 읽기**가 될 거예요.
