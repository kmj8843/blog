...
바로 이럴 때 쓰는 기능이 Wireshark의 **Follow TCP Stream**이에요. 오늘은 흩어진 패킷 조각들을 하나의 완성된 대화문으로 바꿔주는 이 기능의 실전 장면을 같이 열어볼게요.
...
단톡방에서 수많은 사람이 동시에 대화를 나누고 있다고 상상해볼까요?

- **패킷 목록**은 내 화면에 시시각각 올라오는 **모든 메시지의 나열**이에요. (철수, 영희, 민수 대화가 막 섞여 있죠)
- **Follow Stream**은 그중에서 **철수와 영희의 1:1 대화방**만 따로 열어서 처음부터 끝까지 정주행하는 것과 같아요.
