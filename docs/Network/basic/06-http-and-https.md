...
여기서 한 가지만 미리 말해둘게요. 이 글에서는 **HTTP와 HTTPS가 대화 규칙과 보호 방식에서 어떻게 다르게 느껴지는지**에 집중하고, **그 보호 통로를 실제로 누가 어떻게 준비하는지**는 [TLS, SSL, 인증서는 뭐가 다를까요?](07-tls-ssl-and-certificates.md#browser-verification-flow){ data-preview }에서 더 자세히 열어볼 거예요.

---

## 브라우저와 서버의 대화를 주문서로 보면요
...
그리고 *"좋아요, 근데 요즘은 왜 HTTP/3가 TCP 대신 UDP 위에 올라간다고 하죠?"* 가 궁금해졌다면, 심화편 [QUIC은 왜 UDP 위에서 돌아갈까요?](../deep-dive/quic-first-look.md#http3-over-quic){ data-preview }에서 **HTTP가 QUIC 위로 옮겨 탄 큰 구조**를 이어서 볼 수 있어요.
