...
그리고 *"받는 쪽은 괜찮아 보이는데, 왜 보내는 쪽이 네트워크 길 사정을 보며 스스로 더 조심스럽게 보내죠?"* 가 궁금해졌다면, 심화편 [TCP 혼잡 제어는 왜 흐름 제어와 따로 봐야 할까요?](../deep-dive/tcp-congestion-control.md#flow-vs-congestion){ data-preview } 에서 **수신자 여유(`rwnd`)와 송신자 혼잡 한도(`cwnd`)를 어떻게 따로 읽는지** 이어서 볼 수 있어요.
...
그럼 받는 쪽은 핵심적으로 **"아직 101번이 안 왔어. 101번(Ack=101)부터 필요해!"** 라고 계속 대답해요. 초심자 입장에서는 *"빠진 자리 기준으로 ACK가 간다"* 이 감각만 먼저 잡아도 충분해요. 이 끈질긴 요구 덕분에 보내는 쪽은 중간에 뭐가 빠졌는지 정확히 알 수 있죠.
