...
근데요, **처음 읽는 분에게는 여전히 01부터 시작하는 길이 제일 자연스러워요.**
뒤쪽 글은 앞에서 만든 직관을 바탕으로, 실제 필드와 상태, 신호로 번역하는 방식이기 때문이에요.

!!! tip "이렇게 읽으면 제일 덜 헷갈려요"
  - 처음이라면 **01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10 → 11 → 12 → 13** 순서가 가장 편해요.
  - 이미 개념 감은 있다면 **08 → 09 → 10 → 11 → 12 → 13** 흐름으로 들어와도 괜찮아요.
    - 뒤쪽 글부터 읽더라도, 중간에 헷갈리면 다시 **01~07** 쪽으로 돌아가 감을 보충하면 돼요.
...
- **01~07**: "이게 뭐지? 왜 필요하지?" 를 먼저 쉽게 익혀요.
- **08**: 지금까지 본 개념을 네트워크 전체 지도 위에 올려서, 어디쯤 있는 기술인지 연결해봐요.
...
- [TCP vs UDP - 꼼꼼한 친구와 빠른 친구는 뭐가 다를까요?](03-tcp-vs-udp.md){ data-preview }에서는 TCP를 "먼저 인사하는 방식"으로 감 잡고,
- [TCP 3-way handshake는 왜 세 번이나 주고받을까요?](09-tcp-3-way-handshake.md){ data-preview }에서는 그 인사 안에 있는 `SYN`, `ACK`, sequence number를 실제로 열어봐요.
...
이 페이지에서는 아래 목록만 **지금 읽을 수 있는 공식 목록**으로 두고 갈게요.
...
지금 공개된 흐름은 **공유기와 홈 네트워크** 까지 왔어요.
그다음부터는 이제 집 안 문지기 역할을 하는 공유기가, 바깥에서 들어오는 연결까지 어떻게 다루는지로 자연스럽게 넘어가요.

- **14. 포트 포워딩과 들어오는 연결** — 평소엔 닫혀 있는 집 안 문을, 어떤 경우에 왜 특정 장치 쪽으로 열어줘야 할까요?
