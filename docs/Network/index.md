...
description: 패킷부터 포트와 소켓까지, 네트워크 시리즈를 어떤 순서로 읽으면 좋은지 한눈에 정리한 로드맵 페이지예요.
icon: lucide/route
render_macros: true
hide:
  - feedback
...
이 네트워크 시리즈는 일부러 **"작은 조각 → 길 찾기 → 전달 방식 → 이름 찾기 → 앱 찾기"** 순서로 이어지고 있어요.
...
5. 마지막으로 **도착한 뒤 어느 앱으로 들어가는지**까지 이어지거든요.
...
"패킷이 뭐지?" 에서 시작했는데,
읽다 보면 자연스럽게 **"그럼 어디로 가?"**, **"잘 도착한 건 어떻게 알아?"**, **"이름은 또 누가 숫자로 바꿔줘?"**, **"도착한 뒤엔 누구한테 가?"** 같은 궁금증으로 이어지죠.
...
- :material-timer-sand:{ .lg .middle } **다음에 궁금해질 주제**
    ...
    이제는 **어느 컴퓨터로 가는지**, **그 안의 어느 앱으로 가는지**까지 보였죠.
    그다음엔 자연스럽게,
    **브라우저와 서버가 실제로 무슨 규칙으로 대화하는지**가 궁금해져요.
...
근데요, **가장 덜 헷갈리는 길은 여전히 01 → 02 → 03 → 04 → 05** 예요.
...
포트와 소켓까지 읽고 나면 이제 **"어느 컴퓨터로 갈지"**, **"그 안의 어느 앱으로 갈지"** 까지는 알게 돼요.

근데 한 컴퓨터 안에는 브라우저도 있고, 게임도 있고, 메신저도 있잖아요?
그다음엔 브라우저와 서버가 **실제로 어떤 규칙으로 대화를 나누는지** 가 궁금해지죠.

그래서 다음 글은 **"HTTP와 HTTPS"** 쪽으로 이어질 가능성이 커요.

이 흐름까지 이어지면,
인터넷에서 데이터가 **어떤 조각으로 나뉘고 → 어디로 가고 → 어떤 방식으로 전달되고 → 이름이 어떻게 주소로 바뀌고 → 누구한테 전달돼야 하는지** 꽤 큰 그림으로 보이기 시작할 거예요.
...
!!! abstract "이 페이지는 이렇게 쓰면 돼요"
    ...
    - 가장 추천하는 순서는 **01 → 02 → 03 → 04 → 05** 예요.
    - 지금은 **5편 공개**, 포트와 소켓까지 이어져 있어요.
