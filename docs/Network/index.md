...
description: 패킷부터 DNS 레코드까지, 네트워크 시리즈를 어떤 순서로 읽으면 좋은지 한눈에 정리한 로드맵 페이지예요.
icon: lucide/route
render_macros: true
hide:
  - feedback
...
9. 그다음 **TCP 연결이 실제로 어떻게 시작되는지** 보고
10. 이어서 **DNS가 실제로 어떤 종류의 답을 돌려주는지** 까지 보게 되거든요.
...
| 10 | [DNS 레코드](10-dns-records.md){ data-preview } | A, AAAA, CNAME... DNS 레코드는 왜 종류가 여러 갈래일까요? | 읽기 가능 |
...
- :material-check-circle-outline:{ .lg .middle } **이미 나온 글**
    ...
    지금은 **10편**이 공개되어 있어요.
    패킷부터 시작해서 OSI/TCP-IP 지도와 TCP 핸드셰이크, 그리고 DNS 레코드까지,
...
- :material-timer-sand:{ .lg .middle } **다음에 궁금해질 주제**
    ...
    이제 DNS 레코드가 어떻게 쓰이는지까지 봤으니,
    다음엔 집 안과 바깥을 잇는 **공인 IP, 사설 IP, 그리고 NAT** 가 어떻게 움직이는지 궁금해져요.
    그 후엔 실제 패킷을 눈으로 확인하는 **패킷 캡처** 이야기가 기다리고 있어요.
...
- **"DNS 레코드 종류가 왜 이렇게 많아요?"** → 10 먼저 읽기
- **"TLS나 인증서가 왜 필요한지 헷갈려요"** → 07 먼저 읽기
- **"TCP랑 UDP 차이만 빨리 알고 싶어요"** → 03 먼저 읽기

근데요, **가장 덜 헷갈리는 길은 여전히 01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10** 예요.
...
이제 DNS 레코드까지 또렷하게 정리했으니, 조금 더 깊은 기술적 원리들을 계속 따라가보려 해요.

다음엔 **공인 IP, 사설 IP, NAT**가 어떻게 우리 집 네트워크를 인터넷과 이어주는지 살펴보고,
그 후에 **패킷 분석** 같은 실전적인 주제들로 뻗어 나갈 예정이에요.
...
!!! abstract "이 페이지는 이렇게 쓰면 돼요"
    ...
    - 가장 추천하는 순서는 **01부터 10까지** 차례대로 읽는 거예요.
    - 지금은 **10편 공개**, DNS 레코드까지 이어져 있어요.
