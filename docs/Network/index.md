...
지금 공개된 흐름은 이제 **기본 게이트웨이와 첫 번째 도약** 까지 왔어요.
이제 그다음부터는, 그렇게 바깥으로 나간 패킷이 **어디까지 갔는지, 어디서 막혔는지** 를 우리가 어떻게 확인할 수 있는지까지 열어볼게요.

- **19. ICMP, Ping, 그리고 Traceroute** — 패킷이 어디까지 갔는지, 어디서 막혔는지 네트워크는 어떻게 힌트를 줄까요?
