...
지난 글에서 우리는 **OSI 7계층과 TCP/IP 모델**을 보면서, TCP가 **전송 계층**에서 연결을 시작하는 중요한 역할을 맡는다고 봤어요.
그리고 더 앞선 **TCP vs UDP** 글에서는 TCP가 UDP와 다르게 **먼저 인사부터 한다**는 감각도 잡았죠.

즉, 03편이 **"TCP는 시작 전에 확인부터 하는구나"** 라는 직관을 잡아주는 글이었다면, 이번 글은 그 인사 안을 열어서 **실제로 어떤 신호와 숫자가 오가는지** 보는 메커니즘 편이라고 생각하면 돼요.
...
이 부분이 오늘 글의 핵심이에요.
[TCP vs UDP - 꼼꼼한 친구와 빠른 친구는 뭐가 다를까요?](03-tcp-vs-udp.md#tcp) 글에서는 "먼저 인사한다"는 감각을 봤다면, 여기서는 **그 인사 안에 숫자가 어떻게 들어 있는지** 보는 거예요.
