...
!!! note "이 글의 범위"
    ...
    다만 "편해졌다"에서 끝내지는 않을게요. 뒤쪽에서는 Boot가 어떤 재료를 보고 기본값을 고르는지, 그리고 그 기본값이 실무에서 어떤 확인 지점으로 이어지는지도 같이 잡아볼 거예요.
...
컨트롤러가 서비스를 직접 만들고, 서비스가 저장소를 직접 만들면 처음에는 흐름이 눈에 잘 보여요. 하지만 이 구조에서는 클래스가 자기 일을 하기 전에 실행 환경을 조립하는 일까지 떠안아요.

Spring이 줄이려는 부담은 바로 이 지점이에요.

| 코드 안에 섞인 일 | Spring을 쓰면 어디로 옮겨갈까요? |
|---|---|
| 객체를 언제 만들지 결정함 | Spring 컨테이너가 빈(bean) 생성 시점을 관리해요 |
| 어떤 구현을 넣을지 직접 고름 | 설정, Annotation, 조건에 따라 빈 관계를 구성해요 |
| 여러 객체의 생성 순서를 맞춤 | 의존성 그래프를 보고 필요한 순서로 준비해요 |
| 트랜잭션, 보안, 로깅 같은 공통 규칙을 반복함 | 프록시(proxy)와 AOP 같은 구조로 메서드 주변에 적용해요 |

여기서 핵심은 **Spring이 업무를 대신한다**가 아니에요.
...
그래서 이 시리즈는 "Annotation을 외우는 글"이 아니라, **내가 쓴 코드와 프레임워크가 대신 한 일을 나눠보는 글**로 갈 거예요.

---

## 여기부터는 조금 더 깊게 볼게요

Boot의 기본값은 "Spring Boot가 알아서 잘해준다"라는 한 문장으로 끝내면 편해요. 그런데 실무에서는 그 문장이 너무 뭉뚱그려져서 문제를 만들 때가 많아요.

조금 더 정확히 보면 Boot는 보통 이런 재료를 함께 봐요.

| Boot가 보는 재료 | 왜 중요할까요? |
|---|---|
| 클래스패스(classpath)에 들어온 라이브러리 | 웹, 데이터베이스, JSON, 보안 같은 기능 후보를 판단해요 |
| 사용자가 직접 등록한 빈(bean) | 이미 명시한 설정이 있으면 기본 설정이 물러날 수 있어요 |
| 설정 파일과 환경 변수 | 포트, 프로필, 커넥션 정보처럼 실행 모양을 바꿔요 |
| 조건(condition) | "이 클래스가 있을 때만", "이 빈이 없을 때만" 같은 자동 설정의 적용 여부를 가려요 |

그래서 자동 설정(auto-configuration)은 "무조건 켜지는 마법"이 아니라, **조건을 만족할 때 적용되는 기본 설정 후보**에 가까워요.

```mermaid
flowchart LR
    A[스타터 의존성<br/><small>클래스패스에 재료가 들어옴</small>]
    B[자동 설정 후보<br/><small>조건을 보고 적용 여부 판단</small>]
    C[사용자 설정<br/><small>직접 만든 빈과 설정값</small>]
    D[실행 중 컨텍스트<br/><small>최종 빈과 기본값 조합</small>]
...
이 그림에서 중요한 건 `자동 설정 후보`예요. Boot는 스타터가 있다고 해서 항상 같은 객체를 무조건 밀어 넣지 않아요. 조건을 보고, 이미 사용자가 만든 것이 있는지 보고, 최종적으로 컨텍스트에 무엇을 둘지 결정해요.

실무에서는 이 차이가 디버깅 기준이 돼요.

| 보이는 증상 | 먼저 볼 질문 |
|---|---|
| 서버가 예상과 다른 포트로 떠요 | 어떤 설정값이 우선했나요? 명령줄 인자, 환경 변수, 설정 파일 중 무엇이 이겼나요? |
| 내가 만든 설정이 적용되지 않아요 | 빈이 실제로 등록됐나요? 자동 설정 조건 때문에 다른 구성이 들어왔나요? |
| 라이브러리를 넣었더니 갑자기 동작이 생겼어요 | 스타터가 어떤 의존성을 클래스패스에 추가했나요? |
| 테스트에서는 되는데 실행하면 달라요 | 테스트 컨텍스트와 실제 실행 컨텍스트가 같은 빈과 설정을 쓰나요? |

처음 읽을 때는 여기까지 전부 외울 필요 없어요. 다만 Spring Boot를 깊게 읽는 출발점은 이 문장으로 잡으면 좋아요.
