...
내가 `new OrderService()`를 쓴 적이 없는데 `OrderService` 객체가 쓰이고요. `OrderRepository`를 직접 넘긴 적도 없는데 생성자에 들어와요. 나중에는 `@Transactional` 같은 Annotation을 붙였을 뿐인데 트랜잭션이 시작되고 끝나요.
...
!!! note "이 글의 범위"
    오늘은 IoC, DI, AOP의 모든 내부 구현을 파고들지는 않아요. 목표는 Spring Boot 코드를 읽을 때 "이건 객체 생성 책임 이야기구나", "이건 연결 방식 이야기구나", "이건 메서드 주변에 붙는 공통 규칙 이야기구나" 하고 구분하는 거예요. 다만 각 개념이 실무에서 어디서 깨지는지도 함께 볼게요.
...
## 그래서 Annotation 하나만 보고 끝내면 부족해요

Spring Boot를 쓰다 보면 Annotation 하나가 많은 일을 하는 것처럼 보여요.
...
!!! warning "Annotation만 보고 내부 동작을 단정하면 위험해요"
    `@Transactional`, `@Cacheable`, 보안 Annotation처럼 메서드 주변 동작을 바꾸는 기능은 프록시와 호출 경로의 영향을 받아요. "붙였으니 무조건 된다"보다 "누가 이 메서드를 어떤 경로로 호출하나"를 같이 봐야 해요.

---

## 실무에서는 경계가 더 중요해져요

처음에는 IoC, DI, AOP를 각각 한 문장으로 이해해도 좋아요.

- IoC: 객체 생성 제어가 컨테이너로 이동해요.
- DI: 필요한 객체가 바깥에서 들어와요.
- AOP: 메서드 주변에 공통 규칙이 붙어요.

하지만 숙련자가 Spring 코드를 읽을 때는 보통 한 단계 더 깊게 봐요. "무슨 기능인가?"보다 "어느 경계에서 적용되는가?"를 보는 거예요.

| 기준점 | 처음 이해 | 더 깊게 볼 경계 | 자주 만나는 실패 |
|---|---|---|---|
| IoC | 컨테이너가 객체를 만들어요 | 빈 정의 등록, 실제 객체 생성, 초기화, 후처리 시점이 나뉘어요 | 생성 시점에 필요한 설정이나 빈이 아직 없어서 시작 실패 |
| DI | 생성자로 필요한 객체를 받아요 | 같은 타입 빈이 여러 개인지, 선택 가능한 빈인지, 순환 의존성이 있는지 봐야 해요 | `NoSuchBeanDefinition`, `NoUniqueBeanDefinition`, 순환 참조 |
| AOP | 공통 규칙이 메서드 앞뒤에 붙어요 | 호출이 프록시를 지나가는지, 어떤 메서드가 가로채지는지 봐야 해요 | 자기 자신 호출(self-invocation), `final`/`private` 메서드, 프록시 바깥 호출 |

예를 들어 `@Transactional`은 초보자에게는 "DB 작업을 하나로 묶어줘요"로 시작해도 충분해요. 그런데 실무에서는 이 설명만으로 부족해지는 순간이 와요.
...
겉으로는 `saveOrder()`에 `@Transactional`이 붙어 있어요. 그런데 `createOrder()`가 같은 객체 안에서 `saveOrder()`를 직접 부르면, 호출이 프록시를 지나가지 않는 구조가 될 수 있어요. 그러면 기대한 트랜잭션 경계가 생기지 않을 수 있죠.

이걸 외우는 규칙으로만 받아들이면 금방 헷갈려요. 원리는 더 단순해요.

> AOP 기반 기능은 "Annotation이 붙은 메서드가 있느냐"만 보지 말고, "그 메서드 호출이 프록시를 통과하느냐"를 같이 봐야 해요.

트랜잭션 글에서는 여기에 롤백 규칙, 전파(propagation), 격리(isolation), 영속성 컨텍스트까지 붙어서 더 깊어질 거예요. 오늘은 그 전에 Spring을 읽는 기준점만 세우는 단계예요.
...
한 번 더 코드에서 바로 보이는 기준으로 놓아볼게요.

| Spring이 대신 맡은 일 | 연결되는 기준점 |
|---|---|
| 객체를 만들고 관리할 위치가 개발자 코드 밖으로 옮겨감 | IoC |
| 클래스가 필요한 협력 객체를 생성자로 드러내고, 컨테이너가 넣어줌 | DI |
| 메서드 본문 밖에서 트랜잭션, 보안, 로깅 같은 공통 규칙이 적용됨 | AOP |
...
`@SpringBootApplication`은 자동 설정(auto-configuration), 컴포넌트 스캔(component scan), 설정 클래스 역할을 한곳에 묶어주는 대표 Annotation이에요.
...
!!! abstract "오늘 우리가 잡은 세 기준점"
    ...
    - 한 단계 깊게 보면 IoC는 생성 시점, DI는 선택 가능한 빈, AOP는 프록시를 지나는 호출 경계를 함께 읽어야 해요.
