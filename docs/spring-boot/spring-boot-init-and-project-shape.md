...
아직 주문 조회나 결제 같은 기능을 만든 단계는 아니에요. 빌드 도구, Java 버전, 패키지 이름, 실행 방식, 처음 클래스패스에 올라갈 의존성처럼 **코드를 올릴 기본 실행 구조**를 정하는 단계예요.
...
| 처음 보이는 파일이나 선택 | 나중에 이어지는 깊은 의미 |
|---|---|
| `build.gradle` 또는 `pom.xml` | 클래스패스에 어떤 라이브러리가 들어오는지 정하고, 자동 설정 후보를 바꿔요 |
| 스타터(starter) 의존성 | 단일 라이브러리 하나가 아니라, 기능에 필요한 라이브러리 조합과 버전 관리를 함께 가져와요 |
| `OrderApplication.java` 위치 | 컴포넌트 스캔의 자연스러운 시작 범위를 만들어요 |
| `application.properties` | 코드가 아니라 실행 환경에서 바뀌어야 할 값을 분리하는 자리가 돼요 |
| `src/test/java` | 실제 실행 컨텍스트와 테스트 컨텍스트를 비교해볼 출발점이 돼요 |
| `gradlew`, `mvnw` | 팀원과 CI가 같은 빌드 도구 버전으로 실행할 수 있게 해요 |

여기서 가장 중요한 연결은 **의존성 목록과 자동 설정**이에요.
...
```mermaid
flowchart LR
    A[Initializr 선택<br/><small>web, devtools 같은 의존성</small>]
    B[빌드 파일<br/><small>스타터와 버전 관리</small>]
    C[클래스패스<br/><small>실행 시 보이는 라이브러리</small>]
    D[자동 설정 후보<br/><small>조건에 따라 적용</small>]
    E[실행 중 앱<br/><small>웹 서버, MVC, JSON 등</small>]

    A --> B --> C --> D --> E
```

이 흐름을 알면 `build.gradle`을 단순한 설치 목록으로만 보지 않게 돼요. 어떤 스타터를 추가하거나 빼는 일은 "라이브러리 하나를 넣고 빼는 일"이면서 동시에 "Boot가 볼 수 있는 자동 설정 후보를 바꾸는 일"이에요.
...
처음 프로젝트를 만들 때는 "파일이 왜 이렇게 많지?"가 자연스러운 질문이에요. 한 단계 깊게 보면 "이 파일들이 나중에 어떤 실행 결정을 만들지?"가 더 중요한 질문이 돼요.
