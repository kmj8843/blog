...
근데요, 캐시에서 중요한 건 빠른 것보다 먼저 **맞는 응답 사본을 맞는 요청에 주는 것**이에요. 이 응답 사본에는 본문과 응답 헤더가 함께 들어 있어요. 캐시 오염은 캐시가 이번 요청에 맞지 않는 저장된 응답을 선택해 재사용할 때 의심하는 증상이에요.
...
**캐시 오염의 핵심은 본문만 틀리는 게 아니라, 이번 요청에 맞지 않는 응답 사본(본문과 응답 헤더)을 캐시가 선택했다는 데 있어요.**
...
여기서 헷갈리기 쉬운 점은, 캐시가 비교해야 하는 것이 본문만은 아니라는 거예요. **본문이 맞아도 저장된 응답의 CORS 헤더가 이번 요청과 다르면 그 응답 사본을 안전하게 재사용할 수 없어요.**
...
!!! abstract "오늘 우리가 본 것"
    - 캐시 오염은 캐시가 있다는 문제가 아니라, **이번 요청에 맞지 않는 응답 사본(본문과 응답 헤더)을 맞는 것처럼 선택해 재사용하는 장면**이에요.
