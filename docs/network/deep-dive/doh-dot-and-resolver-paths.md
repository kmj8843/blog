...
DNS 질문 이름과 응답 내용은 TLS 안에 숨지만, 연결 바깥의 메타데이터까지 모두 사라지는 건 아니에요. 관찰 위치와 TLS 설정에 따라 이런 신호는 남을 수 있어요.

| 남는 신호 | 왜 남을 수 있나요? |
|---|---|
| 상대 IP 주소와 포트 | 어느 주소로 연결했고 443 또는 853을 썼는지는 경로에서 보일 수 있어요 |
| 연결 시각·지속 시간·전송량 | 암호화돼도 패킷 시각, 방향, 전체 바이트 수 같은 흐름 정보는 남을 수 있어요 |
| TLS 연결 단서 | ECH 사용 여부 같은 환경에 따라 SNI 등 서버 이름 단서가 보일 수도, 가려질 수도 있어요 |
| 클라이언트·프록시·리졸버 로그 | TLS가 끝나는 지점은 설정과 권한에 따라 요청 대상이나 DNS 질문을 기록할 수 있어요 |

다만 크기와 타이밍만 보고 **어떤 도메인을 물었다고 확정할 수 있는 건 아니에요.** 이런 값은 연결을 구분하거나 이상 징후를 찾는 단서이지, 암호화된 DNS 내용 그 자체는 아니죠. 즉 DoH와 DoT는 중간 경로에서 DNS 내용을 읽기 어렵게 만들지만, **내가 어떤 리졸버와 TLS 종료 지점을 믿고 맡기는지**는 여전히 중요한 문제가 돼요.
