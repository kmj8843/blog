...
근데 이 글이 중요한 이유는, UDP가 단순하다는 말을 **정확히 어디까지 믿어야 하는지** 구분해줘야 하기 때문이에요.

- UDP는 정말로 **헤더가 짧은 것**뿐인가?
- 그럼 길이와 체크섬은 왜 아직 남아 있지?
- QUIC, DNS, 실시간 스트리밍처럼 UDP 위에서 중요한 것들이 올라가는데, 그 바닥은 **정확히 얼마나 비워져 있는 구조**지?

오늘은 **UDP 헤더 8바이트**를 32비트 줄 두 줄 위에 펼쳐서, 출발지 포트 · 도착지 포트 · 길이 · 체크섬이 각각 어디 칸에 들어가는지 같이 읽어볼게요. 기본 형식은 [RFC 768](https://www.rfc-editor.org/rfc/rfc768) 을 바탕으로 보고, IPv6에서 UDP 체크섬을 어떻게 봐야 하는지는 [RFC 8200 8.1절](https://www.rfc-editor.org/rfc/rfc8200.html#section-8.1) 기준으로 가볍게 이어볼게요.
...
## 그래서 UDP 헤더는 한마디로 뭐예요?

UDP 헤더는 **정말 필요한 최소 전달 정보만 남긴 짧은 전달표**에 가까워요.

TCP처럼 *"몇 번째 조각인지"*, *"다음엔 몇 번부터 받을지"*, *"연결을 지금 시작하는지"* 같은 운영 메모는 없어요. 대신 최소한 이것만 남겨요.

- 어느 포트에서 어느 포트로 가는지
- 이 데이터그램 전체가 얼마나 되는지
- 전송 중 크게 망가졌는지 최소한으로 확인할 체크섬
...
그러니까 UDP 헤더는 *"아무 정보도 없는 빈 껍데기"* 가 아니라, **정말 필요한 네 칸만 남긴 최소 전달표**라고 보면 돼요. 그리고 이 단순함이 왜 QUIC 같은 상위 프로토콜에게 매력적이었는지도 뒤에서 자연스럽게 이어져요.
...
- TCP 쪽 신호가 실제 캡처 줄에서 어떻게 보이기 시작하는지 같이 보고 싶다면 — [tcpdump 한 줄은 어떻게 읽어야 할까요?](./tcpdump-first-look.md){ data-preview }
