...
즉, [TCP vs UDP](03-tcp-vs-udp.md#tcp-intro){ data-preview }가 **"TCP는 시작 전에 확인부터 하는구나"** 라는 직관을 잡아주는 글이었다면, 이번 글은 그 인사 안을 열어서 **실제로 어떤 신호와 숫자가 오가는지** 보는 메커니즘 편이라고 생각하면 돼요.
...
## SYN, SYN-ACK, ACK는 각각 무슨 말일까요? { #handshake-signals }
...
그리고 *"좋아요, handshake 원리는 알겠어요. 근데 실제 캡처 화면에서는 어느 세 줄이 그 장면이고, 어디서 끊기면 무슨 뜻이죠?"* 가 궁금하다면, 심화편 [tcpdump에서 TCP handshake는 어떻게 보일까요?](../deep-dive/tcp-handshake-in-capture.md#signals-to-read){ data-preview } 에서 **실제 `SYN → SYN-ACK → ACK` 캡처 세 줄을 장면 해석 중심으로** 바로 이어서 볼 수 있어요.
...
이 부분이 오늘 글의 핵심이에요.
[TCP vs UDP - 꼼꼼한 친구와 빠른 친구는 뭐가 다를까요?](03-tcp-vs-udp.md#tcp-intro){ data-preview } 글에서는 "먼저 인사한다"는 감각을 봤다면, 여기서는 **그 인사 안에 숫자가 어떻게 들어 있는지** 보는 거예요.
...
!!! note "한 가지 더"
    TCP 핸드셰이크는 **인증**도 아니고 **암호화**도 아니에요. 상대가 누구인지 확인하는 건 [TLS나 인증서 쪽 이야기](07-tls-ssl-and-certificates.md#why-verification-matters){ data-preview }고, 핸드셰이크는 어디까지나 **전송 계층에서 연결을 시작하는 합의**에 더 가까워요.
