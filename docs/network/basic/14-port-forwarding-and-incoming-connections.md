...
이걸 **CGNAT**라고 불러요. [공인 IP, 사설 IP, 그리고 NAT](11-public-private-ip-and-nat.md#nat-how-it-works){ data-preview }에서 봤던 NAT를 통신사 쪽에서 한 번 더 하는 상황이라고 생각하면 이해하기 쉬워요.

이 경우에는 공유기에서 포트 포워딩만 설정해도 바깥에서 직접 들어오기 어려울 수 있어요. 통신사와 접속 환경에 따라 공인 IP 제공 옵션을 확인하거나, 조건이 맞으면 터널링 서비스나 IPv6 같은 대안을 검토할 수 있어요.

지금은 **"공인 IP를 못 받으면 포트 포워딩만으로는 안 풀릴 수 있구나"** 정도만 잡아두면 충분해요.
...
포트 포워딩은 내가 만든 서비스를 세상에 공개하는 일이에요.
문을 연 뒤에는 **누구의 연결을 허용할지**도 함께 생각해야 하죠.
...
다음 글에서는 [방화벽과 상태 기반 필터링](15-firewall-and-stateful-filtering.md){ data-preview } 이야기를 통해,
