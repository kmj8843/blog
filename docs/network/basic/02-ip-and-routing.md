...
예전 자료에서는 이 숫자를 A/B/C 클래스처럼 나눠 설명하는 경우도 있어요. 여기서는 먼저 IPv4 주소의 큰 모양만 보고, 왜 지금은 클래스보다 `/24`, `/20` 같은 CIDR prefix를 더 중요하게 읽는지는 [심화편의 클래스와 CIDR 역사 글](../deep-dive/classful-addressing-and-cidr-history.md){ data-preview }에서 이어서 열어볼게요.
...
여기서는 라우터가 “다음 어디로 보낼지”를 고른다는 큰 그림만 볼게요. 라우팅 테이블에 여러 경로가 동시에 맞을 때 어떤 줄이 선택되는지는 [Longest Prefix Match 글](../deep-dive/longest-prefix-match-and-route-selection.md){ data-preview }에서 더 깊게 열어볼 수 있어요.
...
다음 글에서는 [**"TCP vs UDP"**](03-tcp-vs-udp.md){ data-preview } 이야기를 해볼게요. 패킷을 보내는 두 가지 성격의 친구가 있어요. 한 명은 꼼꼼하고, 한 명은 빠르고. 둘이 어떻게 다른지 같이 살펴봐요.
