...
여기서는 TTL을 캐시의 유통기한 정도로만 볼게요. DNS 설정을 바꿨는데도 누군가는 예전 주소를 계속 보는 장면이 궁금하다면, 심화편 [DNS TTL과 캐시는 왜 바뀐 주소를 바로 안 보여줄까요?](../deep-dive/dns-ttl-and-cache-staleness.md){ data-preview }에서 재귀 리졸버 캐시와 음성 캐시까지 이어서 볼 수 있어요.
