...
description: 패킷 캡처가 무엇인지, 캡처 도구에서 TCP·DNS·NAT 흔적이 어떻게 보이는지 캡처 위치 중심으로 쉽게 풀어봅니다.
icon: lucide/search
created: 2026-05-12
updated: 2026-06-16
tags:
...
## 어디에서 캡처하느냐가 왜 그렇게 중요할까요? { #capture-location-matters }
...
[공인 IP, 사설 IP, 그리고 NAT는 왜 같이 나올까요?](11-public-private-ip-and-nat.md#nat-how-it-works){ data-preview }에서 봤던 NAT가 이제 여기서 진짜 중요해져요.
...
[DNS는 어떻게 이름을 IP 주소로 바꿀까요?](04-dns.md#dns-role){ data-preview }와 [DNS 레코드는 왜 종류가 여러 갈래일까요?](10-dns-records.md#dns-records-role){ data-preview }에서 본 것처럼,
...
[TCP 3-way handshake는 왜 세 번이나 주고받을까요?](09-tcp-3-way-handshake.md#handshake-signals){ data-preview }에서 봤던
...
이렇게 저장해두면 나중에 다른 분석 도구로 다시 열어볼 수 있어요.
다만 실제로 저장한 `.pcap` 파일에는 주소, 요청 시점, 경우에 따라 민감한 내용이 함께 들어갈 수 있으니, **함부로 공유하지 않는 편이 좋아요.**

### 저장한 캡처 파일은 언제 다시 보기 좋을까요?

패킷을 **줄 단위로만 보는 게 아니라 흐름으로 다시 따라가고 싶을 때** 좋아요. 단순히 한 줄씩 읽는 걸 넘어서, 저장한 캡처를 열어 **특정 연결이 어떻게 이어졌는지** 차근차근 확인하는 식으로 쓰게 되죠.
...
이제 마지막으로, 왜 다들 `tcpdump` 같은 패킷 캡처 도구 이야기를 자주 하는지 감을 잡아볼게요.
...
!!! abstract "오늘 우리가 배운 것"
    ...
    - `tcpdump` 같은 도구는 빠르게 잡아보고, 저장한 캡처 파일은 나중에 다시 열어 흐름을 차분히 따라가 볼 수 있어요.
