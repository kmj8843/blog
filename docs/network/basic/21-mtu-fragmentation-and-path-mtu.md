...
[ICMP, Ping, 그리고 Traceroute](20-icmp-ping-and-traceroute.md){ data-preview }에서,
...
왜 이런 철학이 가능하냐면, IPv6는 애초에 기본 헤더에서 **중간 라우터 단편화 전담 칸**을 빼고, 필요하면 뒤쪽 **확장 헤더**로 넘기는 쪽으로 구조를 다시 짰기 때문이에요. 이 헤더 모양 자체를 보고 싶다면 [IPv6 헤더는 왜 딱 40바이트일까요?](../deep-dive/ipv6-header-anatomy.md#extension-headers){ data-preview }에서 `Next Header` 와 확장 헤더 흐름을 같이 볼 수 있어요.
...
[ICMP, Ping, 그리고 Traceroute](20-icmp-ping-and-traceroute.md){ data-preview }에서 봤듯이, 보안상의 이유로 ICMP를 막아두는 곳이 많아요.
...
작은 요청과 `ping`은 성공하는데 큰 응답만 멈추는 실제 장애 장면이 궁금하다면,
심화편 [작은 요청은 되는데 큰 응답만 멈추는 이유는 뭘까요?](../deep-dive/case-mtu-blackhole.md){ data-preview }에서 패킷 크기, 반복 재전송, ICMP 신호를 이어서 읽어볼 수 있어요.
...
다음 글에서는 TCP의 가장 강력한 특징인 **[TCP 재전송과 신뢰성](22-tcp-retransmission-and-reliability.md){ data-preview }** 이야기를 통해, 끈질기게 데이터를 책임지는 메커니즘을 같이 열어볼게요.
