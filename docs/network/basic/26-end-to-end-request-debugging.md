...
반대로 *"연결은 열렸는데 왜 인증서 경고가 뜨죠? 이름이 안 맞는 건지, 체인이 끊긴 건지 어떻게 읽죠?"* 가 궁금해졌다면, 심화편 [TLS 인증서 체인과 신뢰 오류는 어떻게 읽어야 할까요?](../deep-dive/tls-cert-chain-and-trust-errors.md#trust-errors){ data-preview }에서 **TLS 구간 안에서도 어느 검사가 실패했는지**를 더 구체적으로 이어서 볼 수 있어요.
