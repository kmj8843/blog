...
[DHCP](17-dhcp.md){ data-preview }에서 우리 기기가 네트워크에 붙자마자 자기 IP 주소와 게이트웨이 정보를 어떻게 자동으로 받는지 봤어요.
...
!!! tip "봉투의 진짜 모습이 궁금하다면?"
    이 봉투(이더넷 프레임)가 정확히 몇 바이트고, 안에 어떤 칸들이 있는지 궁금하신가요? [이더넷 프레임과 VLAN 태그 해부하기](../deep-dive/ethernet-frame-and-vlan.md){ data-preview }에서 비트 단위로 자세히 열어볼 수 있어요.
...
이걸 결정하는 기준이 바로 [DHCP](17-dhcp.md){ data-preview }에서 받았던 **서브넷 마스크**예요.
...
[DHCP](17-dhcp.md){ data-preview } 글에서 공유기가 **IP 주소, 서브넷, 기본 게이트웨이**를 같이 준 이유도 여기서 다시 이어져요.
...
다음 글에서는 **[기본 게이트웨이와 첫 번째 도약(First-hop)](19-default-gateway-and-first-hop.md){ data-preview }** 이야기를 통해, 우리 집 패킷이 드넓은 인터넷 바다로 나가는 그 첫 번째 순간을 더 깊게 들여다볼게요.
