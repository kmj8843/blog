...
[Proxy, Reverse Proxy, 그리고 Load Balancer](24-proxy-reverse-proxy-and-load-balancer.md){ data-preview }에서는
...
세세한 운영 팁과 디버깅은 심화편에서 더 열어봐도 충분해요.

---

## 복사본이 가까이에 있으면 뭐가 달라질까요?
...
그러니까 [Proxy, Reverse Proxy, 그리고 Load Balancer](24-proxy-reverse-proxy-and-load-balancer.md){ data-preview }가
**서버 앞단의 입구 정리**였다면,
...
캐시 키를 너무 넓게 잡으면 **다른 사용자에게 엉뚱한 내용이 섞여 보일 수 있어요.**
반대로 너무 잘게 쪼개면 **복사본을 거의 재사용하지 못해 히트율이 떨어질 수 있고요.**

즉, 캐시는 그냥 "저장한다"에서 끝나는 게 아니라,
**무엇을 같은 것으로 볼지 정하는 일**까지 같이 들어 있어요.

이 기준을 헤더와 운영 화면에서 더 자세히 읽고 싶다면, 심화편의 [Cache Key와 Vary](../deep-dive/cache-key-and-vary.md){ data-preview }에서 같은 URL처럼 보여도 언어, 압축, 쿠키 조건 때문에 사본이 어떻게 나뉘는지 이어서 볼 수 있어요.
...
장바구니 상태가 섞인 응답,
개인화된 추천 결과 같은 건
사람마다 다른 내용을 같은 복사본으로 묶지 않도록 더 세심하게 다뤄야 해요.
...
더 깊게 보고 싶다면 [Cache-Control과 Age 헤더 읽기](../deep-dive/reading-cache-control-and-age.md){ data-preview }에서 복사본의 나이를, [ETag와 조건부 요청](../deep-dive/etag-and-conditional-requests.md){ data-preview }에서 같은 버전인지 다시 확인하는 흐름을 이어서 볼 수 있어요.
...
다음 글에서는 **[End-to-End Request Debugging](26-end-to-end-request-debugging.md){ data-preview }** 이야기를 해볼게요.
