...
[HTTP와 HTTPS는 뭐가 다를까요?](06-http-and-https.md){ data-preview }에서 우리는 **HTTP** 와 **HTTPS** 의 차이를 봤어요.
...
## TLS, SSL, 인증서는 각각 뭐가 다를까요? { #tls-basics }
...
## 근데 왜 굳이 이런 확인 과정이 필요할까요? { #why-verification-matters }
...
## 그럼 브라우저는 실제로 어떤 순서로 확인할까요? { #browser-verification-flow }
...
여기서는 이 흐름을 **큰 그림**으로만 볼게요. 메시지 순서를 먼저 펼쳐보고 싶다면 [TLS 1.3 핸드셰이크](../deep-dive/tls13-handshake-anatomy.md#full-handshake){ data-preview }와 [단계별 핸드셰이크 장면](../deep-dive/tls-handshake-step-by-step.md#scene-first-look){ data-preview }으로 이어가면 돼요.

인증서 경고를 좁혀보고 싶다면 [인증서 체인과 신뢰 오류](../deep-dive/tls-cert-chain-and-trust-errors.md#signals-to-read){ data-preview }와 [인증서 만료 장애](../deep-dive/case-cert-expired-incident.md#signals-to-read){ data-preview }를 보면 좋아요. 인증서를 꺼내기 전에 서버 이름을 어떻게 알리는지가 궁금할 때는 [SNI, ESNI, ECH](../deep-dive/sni-and-esni-ech.md#why-name-before-certificate){ data-preview }로 이어갈 수 있어요.
...
다음 글에서는 네트워크의 전체 지도, [**"OSI 7계층과 TCP/IP 모델"**](08-osi-and-tcp-ip-layers.md){ data-preview } 이야기를 해볼게요.
