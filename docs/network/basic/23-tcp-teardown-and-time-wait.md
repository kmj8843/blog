...
심화편 [TCP 상태 머신: 연결의 탄생부터 소멸까지의 일대기](../deep-dive/tcp-state-machine.md#termination-states){ data-preview }를 보면, 이 `TIME-WAIT`가 종료 과정의 수많은 상태 중에서도 왜 그렇게 특별하게 다뤄지는지 더 깊은 이유를 확인할 수 있어요.
...
다만 `TIME-WAIT` 개수가 많다는 사실만으로 포트 고갈을 확정할 수는 없어요. 실제 장애에서는 심화편 [TIME-WAIT가 많을 때 정말 포트가 고갈된 걸까요?](../deep-dive/case-time-wait-port-exhaustion.md){ data-preview }처럼 임시 포트 범위, 같은 목적지로 몰리는 4-tuple, 새 연결 오류까지 함께 확인해야 해요.
